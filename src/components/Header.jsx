...
const Header = ({ currentUser, onLogout, isCollapsed, setIsCollapsed }) => {
  ...
  const menuDropdownRef = useRef(null); // Ref for the main menu dropdown
  ...
  const handleTabDropdownClick = (tabId, e) => {
    e.stopPropagation(); // Prevent event from bubbling up
    setActiveDropdown(tabId === activeDropdown ? null : tabId); // Toggle dropdown
    setActiveSubmenu(null); // Reset submenu when dropdown changes
  };
                  ...
                  <button
                    onClick={(e) => {
                      if (tab.items && tab.items.length > 0) {
                        handleTabDropdownClick(tab.id, e);
                      } else {
                        handleTabClick(tab);
                      }
                    }}
                    className={`group relative px-4 py-2 text-sm font-medium transition-colors duration-200 flex items-center space-x-2 rounded-md ${
                      activeTab === tab.id || activeDropdown === tab.id
